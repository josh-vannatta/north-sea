...
var coreContent = function () {
  ...
  return {
    ...
    more: [{ title: 'About Us', link: '/about' }]
  };
}();
...
var coreHeader = function () {
  ...
  function renderSubsection(content, base) {
    ...
    if (content) content.forEach(function (item) {
      var el = JSX.html('div', { className: 'shop-section pointer animated fadeIn' }, [JSX.html('div', { className: 'passive' }, [JSX.html('figure', { className: 'shop-figure mb-5' }, [JSX.html('img', { src: '' + base + item.img }), JSX.html('div', { className: 'plane-shadow auto' })]), JSX.html('h3', { className: 'display-m text-center' }, [item.title])])]);
...
var landingPage = function () {
  ...
  function onLoad() {
    ...
    promoCarousel.connect(promoVideo).newContent(promos).render('#promo-carousel');

    testimonials.render('#testimonials', tList);

    instagram.connect(ENV.instagram.token).getUserContent().then(function (response) {
      return instagram.renderTiles('#insta-feed', response.data);
    });
  }
    ...
    var i = JSX.html('figure', {
      ...
      onClick: function onClick() {
        ...
        removeContent().then(function () {
          return generateContent(_this2, index);
        });
      }
...
var ENV = function () {
  ...
  return {
    ...
    // https://api.instagram.com/oauth/authorize/?client_id=9debc23f0b5f43c189546dad7c3021f7&redirect_uri=http://127.0.0.1:8000/&response_type=token
  };
}();
...
(function () {
  ...
  var routes = [{ path: '/', controller: landingPage }, { path: '/home', controller: landingPage }];
  ...
  $(document).ready(function () {
    ...
    routes.some(function (route) {
      if (route.path === pathName) route.controller.load();
      return true;
    });
  });
})();
  ...
  _createClass(VideoCarousel, [{
    ...
    value: function bind() {
      ...
      this.controls.previous.mouseenter(function () {
        _this3.previewSelectedVideo(_this3.videoList.length - 1);
      });
      this.controls.previous.mouseleave(function () {
        return _this3.resetToActive();
      });
      this.controls.previous.click(function () {
        _this3.loadSelectedVideo(_this3.controls.previous, _this3.videoList.length - 1);
      });

      this.controls.next.mouseenter(function () {
        return _this3.previewSelectedVideo(1);
      });
      this.controls.next.mouseleave(function () {
        return _this3.resetToActive();
      });
      this.controls.next.click(function () {
        return _this3.loadSelectedVideo(_this3.controls.next, 1);
      });
    }
  }, {
    key: 'resetToActive',
    value: function resetToActive() {
      ...
      this.player.play();
      setTimeout(function () {
        _this4.controls.preview.removeClass('active');
        _this4.controls.content.html(_this4.render(_this4.videoList[_this4.currentVideo].content));
      }, 200);
    }
  }, {
    ...
    value: function previewSelectedVideo(offset) {
      ...
      setTimeout(function () {
        _this5.controls.preview.addClass('active');
        _this5.controls.content.html(_this5.render(selected.content));
      }, 200);
      setTimeout(function () {
    ...
    value: function loadSelectedVideo(el, offset) {
      ...
      setTimeout(function () {
        ...
        _this6.controls.preview.removeClass('active out');
      }, 2000);
  ...
  _createClass(VideoControls, [{
    ...
    value: function bind() {
      ...
      this.controls.pause.click(function () {
        return _this7.pauseVideo();
      });
      this.controls.play.click(function () {
        return _this7.playVideo();
      });

      this.controls.replay.click(function () {
        _this7.player.jumpTo(0);
        _this7.playVideo();
      });

      this.controls.seekBar.change(function () {
        _this7.player.jumpTo(Math.floor(_this7.player.prop('duration') * _this7.controls.seekBar.val() / 100));
      });

      this.player.video.addEventListener('timeupdate', function () {
        _this7.controls.seekBar.val(100 / _this7.player.prop('duration') * _this7.player.prop('currentTime'));
      });

      this.controls.expand.click(function () {
        _this7.player.fullscreen();
        _this7.playVideo();
      });
    }
  }, {
    ...
    value: function conceal() {
      ...
      setTimeout(function () {
        if (!_this8.focus) {
          _this8.concealed = true;
          _this8.el.addClass('concealed');
        }
      }, 1000);
    }
  }]);
